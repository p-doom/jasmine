...
def generate_episodes(num_episodes, split, start_level, env_name):
    ...
    episode_metadata = []
    file_idx = 0
    output_dir_split_npy = os.path.join(args.output_dir, "npy", split)
    output_dir_split_arr_rec = os.path.join(args.output_dir, "array_record", split)
    os.makedirs(output_dir_split_npy, exist_ok=True)
    os.makedirs(output_dir_split_arr_rec, exist_ok=True)
    ...
    while episode_idx < num_episodes:
        ...
        if step_t + 1 >= args.min_episode_length:
            ...
            episode_path = os.path.join(
                output_dir_split_npy, f"episode_{episode_idx}.npy"
            )
            np.save(episode_path, episode_data)

            # save as array record
            episode_path = os.path.join(
                output_dir_split_arr_rec, f"episode_{episode_idx}.array_record"
            )
...
def main():
    ...
    with open(os.path.join(args.output_dir, "npy", "metadata.json"), "w") as f:
        json.dump(metadata, f)

    with open(os.path.join(args.output_dir, "array_record", "metadata.json"), "w") as f:
        json.dump(metadata, f)
